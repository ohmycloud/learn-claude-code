...
Subagents (s04) are disposable: spawn, work, return summary, die. No identity, no memory between invocations.
Background tasks (s08) run shell commands but can't make LLM-guided decisions.
...
```python
def spawn(self, name: str, role: str: str, prompt: str) -> str:
...
```bash
cd learn-claude-code
uv run python s09-agent-teams.py
```
